...
ll a[N],n,mod,l,r,q,ans,m;
char c;
ll mod_ (ll a){return 1ll*((a%mod)+mod)%mod;}
nod add(nod a,nod b){
nod ret;
 for(int i=0;i<12;i++){
...
int main(){
    ...
    while(q--){
      ...
      nod ret=rsq(1,1,n,l,r);
      ans=(ret.frq[m]);
      printf("%lld\n",ans);}
      else{
        scanf("%lld%lld",&l,&r);
        if(c=='+')
              {a[l]+=r;update(1,1,n,l,r);printf("%lld\n",a[l]);}
        if(c=='-')
              {r=(a[l]>=r)*r;a[l]-=r;update(1,1,n,l,r);printf("%lld\n",a[l]);}
      }
    }
    return 0;
}
